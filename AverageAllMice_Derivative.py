...
def load_sessions(path = r"/Users/shivasenthilkumar/Desktop/Robinson_Lab/Scripts/TDT_Scripts/DATA/Low Light"): 
...
#find the minimum value from the beginning of array to the peak
...
minvalue = np.min(begtopeak)
#must be the minimum closest to the max peak
horizonalline = minvalue*np.ones(len(newtmArray))
fristminvalue_index = np.where(begtopeak == minvalue)
fristminvalue_index = int(fristminvalue_index[0]) 

#finds all the intersection points
outside = rngArray[Index_of_Max:-1]
subhorizontalline = horizonalline[Index_of_Max:-1]
idx = np.argwhere(np.diff(np.sign(subhorizontalline - outside))).flatten()

secondminindex = Index_of_Max+idx[0] #based off TTL to Index Final (newtm and rngArray)
secondmin = newtmArray[secondminindex] #returns the second minimum value  

finaltm = newtmArray[(fristminvalue_index):secondminindex]
finalyvalue = rngArray[(fristminvalue_index):secondminindex]

ax3 = fig.add_subplot(513)
x2_axis = finaltm
y2_axis = finalyvalue
p6 = ax3.plot(x2_axis, y2_axis, linewidth=2, color='green', label='dLight') #line graph
p8 = ax3.axvline(x=0, linewidth=3, color='slategray', label='Light Onset')
p9 = plt.axhline(y=minvalue, color='r', linestyle='-')
ax3.set_ylabel('z-Score')
ax3.set_xlabel('Seconds')
ax3.set_xlim(0, 1)
ax3.set_title(exp_con)

ax4 = fig.add_subplot(514)
x3_axis = finaltm
y3_axis = np.gradient(y2_axis)
p10 = ax4.plot(x3_axis, y3_axis, linewidth=2, color='green', label='dLight') #line graph
ax4.set_ylabel('Slop of Z-Score Peak')
ax4.set_xlabel('Seconds')
ax4.set_xlim(0, 1)
ax4.set_title(exp_con)

ax5 = fig.add_subplot(515)
x4_axis = finaltm
y4_axis = np.gradient(y3_axis)
p10 = ax5.plot(x4_axis, y4_axis, linewidth=2, color='green', label='dLight') #line graph
ax5.set_ylabel('Slop of Z-Score Peak')
ax5.set_xlabel('Seconds')
ax5.set_xlim(0, 1)
ax5.set_title(exp_con)

print (np.max(y3_axis)*1000)
print (np.max(y4_axis)*1000)

shiva = np.where(y4_axis == np.max(y4_axis))
shiva = int(shiva[0])

Max_Peak_Time = ttime[-1]
FirstMin_Peak_Time = x4_axis[shiva] #based off of teh second derivative
Difference = (Max_Peak_Time - FirstMin_Peak_Time)*1000
